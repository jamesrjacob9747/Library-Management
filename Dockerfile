...
# Copy backend (source + all node_modules including prisma CLI for db push)
WORKDIR /app/backend
...
# ── Initialize PostgreSQL if fresh volume ──
if [ ! -s "$PGDATA/PG_VERSION" ]; then
  echo "[init] Creating PostgreSQL data directory..."
  su-exec postgres initdb -D "$PGDATA"
  # Only listen on localhost (container-internal)
...
su-exec postgres psql -tc "SELECT 1 FROM pg_database WHERE datname='${DB_NAME}'" \
  | grep -q 1 || su-exec postgres createdb "$DB_NAME"
echo "[init] Database '${DB_NAME}' ready."

# ── Push Prisma schema ──
echo "[init] Pushing Prisma schema..."
cd /app/backend
...
NODE_ENV=production PORT=3001 API_KEY="${API_KEY}" FRONTEND_URL="http://localhost" \
  node src/app.js &
NODE_PID=$!
